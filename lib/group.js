...
exports.Group = Class({
  ...
  _sidebarOn: {
    ...
    visitPage: function (url) {
      var tab = this.tabSet.tabWithUrl(url);
      if (tab) {
        tab.activate();
        return;
      }
  ...
  _hubOn: {
    ...
    "tab-init": function (msg) {
      ...
      // FIXME: I don't see why this notification makes sense; this
      // message can happen at any time (possibly because someone else
      // joined).
      notify({
        title: msg.peer.name,
        text: "joined this page."
    ...
    pushPresenting: function (msg) {
      ...
      var tab = this.tabSet.tabWithUrl(url);
      console.log("tried to find tab with URL", url, !!tab);
      if (tab) {

...
exports.Group = Class({
  ...
  initialize: function (groupName, selfIdentity) {
    ...
    this.sidebar = Sidebar({
      ...
      onAttach: function (worker) {
        ...
        setHandlers(worker.port, self._sidebarOn, self);
  ...
  _sidebarOn: {
    ...
    join: function (url, tabid) {
      console.log("joined; opening:", url, tabid);
      tabs.open({
        url: url,
        onOpen: function (tab) {
          tab.shareTabId = tabid;
        }
      });
      this.send({
        type: "joined",
    ...
    joinMirror: function (tabid) {
      console.log("join mirror; opening:", tabid);
      tabs.open({
        url: data.url("mirror/blank.html") + "?tabId=" + encodeURIComponent(tabid),
        onOpen: (function (tab) {
          tab.mirrorTabId = tabid;
          this.send({
            type: "joinedMirror",
            tabId: tabid
          });
  ...
  _hubOn: {
    ...
    joined: function (msg) {
      for each (var tab in tabs) {
        if (tab.id == msg.tabId) {
          tab.shareTabId = msg.tabId;
          tab.hotdishWorker.port.emit("share", tab.shareTabId);
          break;
        }
      }
      // FIXME: notify
    ...
    joinedMirror: function (msg) {
      console.log("Looking for tab", msg.tabId, Array.prototype.map.call(tabs, function (t) {return t.id;}));
      for each (var tab in tabs) {
        if (tab.id == msg.tabId) {
          tab.hotdishWorker.port.emit("mirror");
          tab.isMirroring = true;
    ...
    "mirror-doc": function (msg) {
      for each (var tab in tabs) {
        if (tab.mirrorTabId == msg.tabId) {
          tab.hotdishWorker.port.emit("mirror-doc", msg);
        }
      }
    },
    "mirror-fault": function (msg) {
      for each (var tab in tabs) {
        if (tab.id == msg.tabId) {
          tab.hotdishWorker.port.emit("mirror-fault");
          break;
        }
      }
    }
  ...
  _workerOn: {
    ...
    ready: function (worker) {
      ...
      try {
        worker.port.emit("init", {
          ...
          tabId: worker.tab.id,
          viewerScript: data.load("mirror/viewer.js")
        });
        if (worker.tab.shareTabId) {
          worker.port.emit("share", worker.tab.shareTabId);
        }
        if (worker.tab.isMirroring) {
          worker.port.emit("mirror");
        }
        if (worker.tab.onReloadSendMirror) {
          worker.port.emit("mirror-doc", worker.tab.onReloadSendMirror);
          worker.tab.onReloadSendMirror = undefined;
        }
      } catch (e) {
    ...
    emitMirror: function (worker, msg) {
      msg.tabId = worker.tab.id;
      msg.type = "mirror-doc";
      msg.url = worker.tab.url.replace(/#.*/, "");
      this.hub.send(msg);
    },
    mirrorReload: function (worker, msg) {
      /* This happens when a tab needs to be reloaded, and the message
         re-processed */
      // FIXME: handle any interim messages too
      worker.tab.onReloadSendMirror = msg;
      worker.tab.reload();
    },
    mirrorFault: function (worker) {
      this.hub.send({
        type: "mirror-fault",
        tabId: worker.tab.mirrorTabId
      });
    }
  },

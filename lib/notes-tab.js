...
exports.setupNotesTab = function (group) {
  ...
  if (_notestabs[groupid]) {
    ...
    if (! found) {
      ...
      if (windows.activeWindow != group.activeSDKWindow) {
        restoreActive = windows.activeWindow;
...
exports.teardownNotesTab = function (group) {
  var groupid = group.groupId;
  if (_notestabs[groupid]) {
    _notestabs[groupid].close();
    delete _notestabs[groupid];
  }
};
